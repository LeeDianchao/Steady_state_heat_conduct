...
subroutine SOR_sub(n, tol, maxiter, iter, dumax, nthreads)
    ! Implements SOR iteration on n by n grid
    ...
    ! two dimension array!双精度二维

    real(kind=8), dimension(:), allocatable :: x,y
    ! one dimension array!双精度一维

    real(kind=8) :: h
    integer :: i,j

    ! allocate storage for boundary points too:      ( u(i,j):new     uold:old)
    allocate(x(0:n+1), y(0:n+1), u(0:n+1, 0:n+1), uold(0:n+1, 0:n+1), &
             f(0:n+1, 0:n+1))

	!open File for save datas
	...
	!set other point temperature(初始温度)
    do j=1,n
        do i=1,n
            ! source term:
            f(i,j) = 0.d0
	...
	!create threads for Parallel Computing(并行)
	...
	do j=1,n
		...
		u(i,j) = 1.950d0 * u(i,j) - 0.950d0 * uold(i,j)
